...
export async function GET(request: NextRequest) {
    ...
    try {
        // Kullanıcının BIST varlıklarını getir
        const bistAssets = await db
            .select()
            .from(assets)
            .where(and(
        ...
        for (const asset of bistAssets) {
            try {
                // BIST fiyatını çek
                const symbol = asset.symbol || asset.name;
                const marketPriceResponse = await fetch(
                    `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/prices/latest?symbol=${symbol}&type=STOCK`
                );
        ...
        console.error('BIST assets update error:', error);
        return NextResponse.json({
            success: false,
            error: error instanceof Error ? error.message : 'Unknown error'
        }, { status: 500 });
